...
def get_samples_ticks(path, path2, day_begin, day_end, time_step):
    ...
    X = [np.zeros((num_codes * (mem_data[0].shape[1] - time_step + 1 - 2), 480, 28)) for i in range(time_step)]
    ...
    np.save('/data/nvme/yingda/x_201812', X)
    np.save('/data/nvme/yingda/y_201812', Y)


def get_samples_days(path, path2, day_begin, day_end, time_step):
    day_list = estab_day_list(day_begin, day_end)
    # print(day_list)
    label_data = get_lab(path2, start=str(day_begin), end=str(day_end)).values
    codes_index1 = label_data[:, 0]  # array
    ...
    i = 0
    for date in day_list:
        path_data_olh = path + '/' + 'olh/' + str(date) + '.npy'
        if not os.path.exists(path_data_olh):
            continue
        if i == 0:
            mem_data2 = np.load(path_data_olh)
        else:
            mem_data2 = np.dstack((mem_data2, np.load(path_data_olh)))
        i += 1

    num_codes = len(list(codes_share.keys()))
    X2 = np.zeros((num_codes * (mem_data2[0].shape[1] - time_step + 1 - 2), time_step, 3))

    for i0, code in enumerate(list(codes_share.keys())):
        for i in range(mem_data2[code].shape[1] - time_step + 1 - 2):
            for j in range(time_step):
                mem_data2[code, :, i + j][np.isnan(mem_data2[code, :, i + j])] = 0
                X2[i0 * (mem_data2[code].shape[1] - time_step + 1 - 2) + i, j, :] = mem_data2[code, :, i + j]
    # print(X2)
    np.save('/data/nvme/yingda/x_olh_201812', X2)
...
def get_samples(path, path2, day_begin, day_end, time_step):
    ...
    X = [np.zeros((num_codes * (mem_data[0].shape[1] - time_step + 1 - 2), 480, 28)) for i in range(time_step)]
    Y = np.zeros((num_codes * (mem_data[0].shape[1] - time_step + 1 - 2), time_step, 1))
    X2 = np.zeros((num_codes * (mem_data[0].shape[1] - time_step + 1 - 2), time_step, 3))
    ...
    for i0, code in enumerate(list(codes_share.keys())):
        for i in range(mem_data[code].shape[1] - time_step + 1 - 2):
            for j in range(time_step):
                ...
                X[j][i0 * (mem_data[code].shape[1] - time_step + 1 - 2) + i, :, :] = mem_data[code, :, i + j].reshape(
                    480, 28)
                X2[i0 * (mem_data[code].shape[1] - time_step + 1 - 2) + i, j, :] = mem_data2[code, :, i + j]
                Y[i0 * (mem_data[code].shape[1] - time_step + 1 - 2) + i, j, 0] = \
                label_data[int(np.argwhere(codes_index1 == codes_share[code]))][i + j + 2] if not \
                    np.isnan(label_data[int(np.argwhere(codes_index1 == codes_share[code]))][i + j + 2]) else 0
    np.save('/data/nvme/yingda/x_6', X)
    np.save('/data/nvme/yingda/x_olh_6', X2)
    np.save('/data/nvme/yingda/y_6', Y)


def generate_arrays_from_files(path, path2, day_begin, day_end, time_step, batch_size=64):
    day_list = estab_day_list(day_begin, day_end)
    # print(day_list)
    label_data = get_lab(path2, start=str(day_begin), end=str(day_end)).values
    codes_index1 = label_data[:, 0]  # array
    ...
    for date in day_list:
        ...
        if i == 0:
            ...
            d3, d4 = np.load(path_data_olh).shape
        i += 1
    mem_data = np.zeros((d1, d2, i))
    mem_data2 = np.zeros((d3, d4, i))
    i = 0
    for date in day_list:
        path_data = path + '/' + str(date) + '.npy'
        path_data_olh = path + '/' + 'olh/' + str(date) + '.npy'
        ...
        mem_data2[:, :, i] = np.load(path_data_olh)
        i += 1

    X = [np.zeros((batch_size, 480, 28)) for i in range(time_step)]
    Y = np.zeros((batch_size, time_step, 1))
    X.append(np.zeros((batch_size, time_step, 3)))
    i0 = 0
    while True:
        ...
        for i in range(mem_data[code].shape[1] - time_step + 1 - 2):
            for j in range(time_step):
                ...
                X[j][(i0 * (mem_data[code].shape[1] - time_step + 1 - 2) + i) % batch_size, :, :] = mem_data[code, :,
                                                                                                    i + j].reshape(480,
                                                                                                                   28)
                X[time_step][(i0 * (mem_data[code].shape[1] - time_step + 1 - 2) + i) % batch_size, j, :] = mem_data2[
                                                                                                            code, :,
                                                                                                            i + j]
...
class conv1D_lstm_model:
    ...
    def establish(self, day_dim, tick_len, x1_dim, x2_dim):
        ...
        inputa.append(Input(shape=(day_dim, x2_dim)))
        x3 = BatchNormalization(axis=1)(inputa[day_dim])  # axis=2

        x2 = concatenate([x2, x3], axis=2)
        # print(x2)
...
if __name__ == '__main__':
    ...
    # generator = generate_arrays_from_files(path, path2, 20190201, 20190230, 5)
    # conv_lstm = conv1D_lstm_model()
    # conv_lstm.establish(5, 480, 28, 3)
    # conv_lstm.train(generator)
    # test_x = list(np.load('/data/nvme/yingda/x_3.npy'))
    # test_x_olh = np.load('/data/nvme/yingda/x_olh_3.npy')
    # test_x.append(test_x_olh)
    # test_y = np.load('/data/nvme/yingda/y3.npy')[:, 4, 0]
    ...
    # get_samples_days(path, path2, 20181201, 20181231, 5)
    get_samples(path, path2, 20190601, 20190630, 5)
