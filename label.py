...
def get_ret( path = path2, start = '20160503', end = '20160513' ):
    n=0
    ...
    for fpathe,_,fs in os.walk(path):
            for f in fs:
                ...
                # print(tmp_path)
                pattern1=re.compile(r'(201[6-9]\/[0-9]+\/[0-9]+\/)+zz500+(\.[a-z]+)$')
...
def get_lab(path=path1,start='20160503',end='20160513'):
    re_data=[]
    for fpathe,_,fs in os.walk(path):
        for f in fs:
            tmp_path=os.path.join(fpathe,f)
    ...
    ret = get_ret(start = start, end = end)
