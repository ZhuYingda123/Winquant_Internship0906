...
path_top2000 = '/data/stock/newSystemData/rawdata/universe_top2000'


def read_day_universe_stock(day_begin, day_end):
    day_list = estab_day_list(day_begin, day_end)
    codes = {}
    n = 0
    for date in day_list:
        stock_path = os.path.join(path_top2000, str(date))
        try:
    ...
    codes = {key:codes[key] for key in codes if codes[key]==n}
    return list(codes)
...
def generate_arrays_from_files(path, path2, day_begin, day_end, time_step, batch_size=512):
    day_list = estab_day_list(day_begin, day_end)
    codes2000 = read_day_universe_stock(day_begin, day_end)
    # print(day_list)
    label_data = get_lab(path2, start=str(day_begin), end=str(day_end)).values
    codes_index1 = label_data[:, 0]  # array
    label_data = label_data[:, 1:]
    ...
    # num_codes = array_data.shape[0]
    for j, code in enumerate(codes_index2.keys()):
        if (code in codes_index1) and (code in codes2000):
            codes_share[j] = code
...
class conv1D_lstm_model:
    ...
    def establish(self, day_dim, tick_len, x1_dim, x2_dim):
        shared_conv_layer1 = Conv1D(32, 15, activation='relu', kernel_initializer='random_normal')
        shared_conv_layer2 = Conv1D(32, 15, activation='relu', kernel_initializer='random_normal')
        shared_maxpool_layer = MaxPooling1D(3)
        shared_conv_layer3 = Conv1D(16, 15, activation='relu', kernel_initializer='random_normal')
        shared_conv_layer4 = Conv1D(16, 15, activation='relu', kernel_initializer='random_normal')
...
if __name__ == '__main__':
    ...
    # codes = read_day_universe_stock(20190301, 20190430)
    # print(codes)

    generator = generate_arrays_from_files(path, path2, 20190101, 20190231, 5)
    conv_lstm = conv1D_lstm_model()
    conv_lstm.establish(5, 240, 28, 0)
    conv_lstm.train(generator)
    test_x = list(np.load('/data/nvme/yingda/x_3_240.npy'))
    test_y = np.load('/data/nvme/yingda/y_3.npy')[:, 4, 0]
    pred_y = conv_lstm.predict(test_x)[:, 4, 0]
    p1, p2 = cal_corr(test_y, pred_y)
    print(str(20190101)+' and '+str(20190231))
    print("batchsize:512")
    print(p1, p2)
